...
from .models import Restaurant, FoodItem, CartItem, Order, OrderItem, Payment, Review, Notification
...
def restaurant_detail(request, pk):
    ...
    notifications = Notification.objects.filter(has_read=False)
...
def payment_method(request):
    ...
    return render(request,"marketplace/order/payment_method.html")

@login_required
def place_paystack_order(request):
    cart_items = CartItem.objects.filter(user=request.user)
    ...
    if request.method == "POST":
        ...
        if not address:
            ...
            return redirect('place_paystack_order')
        try:
            ...
            callback_url = request.build_absolute_uri("/verify_paystack_payment/")
            data = {
...
@login_required
def place_flutterwave_order(request):
    cart_items = CartItem.objects.filter(user=request.user)
    ...
    if request.method == "POST":
        ...
        if not address:
            messages.error(request, "Delivery address is required.")
            return redirect('place_flutterwave_order')
        try:
            # Debugging cart items
            print(f"[DEBUG] Found {cart_items.count()} cart item(s) for user {request.user}.")
            ...
            flutterwave_secret_key = settings.FLW_SECRET_KEY
            headers = {
                "accept": "application/json",
                "Authorization": f"Bearer {flutterwave_secret_key}",
                "Content-Type": "application/json"
            }
            print(f"[DEBUG] Created Header  {headers}.")
            callback_url = request.build_absolute_uri("/verify_flutterwave_payment/")
            ...
            data = {
                ...
                "customizations": {
                    "title": "Tastify Food Order",
                    "description": "Payment for your delicious meal",
                    "logo": "https://example.com/logo.png"
                }
            }
            
            print(f"[DEBUG] Created Data  {data}.")
...
def mark_as_read(request, notification_id):
    ...
    notification.save()
    return redirect("mark_as_read", notification_id=notification_id)